Page({
  ...
  onLoad: function (options) {
    ...
    this.selection(options.waresId);//商品规格选择
    ...
    wx.request({
      ...
      success(res) {
        ...
        if (self.data.commodityAttr.length == 1) {
          for (var i = 0; i < self.data.commodityAttr[0].attrValueList.length; i++) {
            self.data.attrValueList[i].selectedValue = self.data.commodityAttr[0].attrValueList[i].attrValue;
          }
          self.setData({
            attrValueList: self.data.attrValueList
          });
